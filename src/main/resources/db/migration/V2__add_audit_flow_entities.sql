...
-- Create audit_flow_def table (idempotent)
DO $$
BEGIN
    -- Create table if it doesn't exist
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'audit_flow_def') THEN
    ...
    ELSE
        ...
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'audit_flow_def' AND column_name = 'entity_type') THEN
            ALTER TABLE audit_flow_def ADD COLUMN entity_type VARCHAR(50);
            CREATE INDEX IF NOT EXISTS idx_audit_flow_entity_type ON audit_flow_def(entity_type);
        END IF;
    END IF;
END $$;

-- Create audit_step_def table (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'audit_step_def') THEN
...
-- Create audit_instance table (idempotent)
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'audit_instance') THEN
