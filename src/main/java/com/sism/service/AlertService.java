...
public class AlertService {
    ...
    public BigDecimal calculateExpectedPercent(Indicator indicator, AlertWindow window) {
        ...
        // Sum up weights of milestones that should be completed by cutoff date
        for (Milestone milestone : milestones) {
            if (milestone.getDueDate() != null && !milestone.getDueDate().isAfter(cutoffDate)) {
                expectedPercent = expectedPercent.add(
                    milestone.getWeightPercent() != null ? milestone.getWeightPercent() : BigDecimal.ZERO
                );
            }
        }

        return expectedPercent.setScale(2, RoundingMode.HALF_UP);
    }
    ...
    public List<AlertEventVO> generateAlertsForWindow(Long windowId) {
        ...
        List<Indicator> indicators = indicatorRepository.findByStatus(IndicatorStatus.ACTIVE);
    ...
    public Page<AlertEventVO> getAlertsByTargetOrg(Long orgId, Pageable pageable) {
        Page<AlertEvent> events = alertEventRepository.findByTargetOrg(orgId, pageable);
        return events.map(this::toAlertEventVO);
    }
