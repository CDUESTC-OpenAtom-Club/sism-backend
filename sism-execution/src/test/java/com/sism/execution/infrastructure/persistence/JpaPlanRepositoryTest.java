...
import com.sism.strategy.infrastructure.persistence.JpaPlanRepository;
import com.sism.strategy.infrastructure.persistence.JpaPlanRepositoryInternal;
...
class JpaPlanRepositoryTest {
    ...
    void shouldUseStatusSpecificPageQueryWhenStatusIsProvided() {
        ...
        when(jpaRepository.findPageByStatus(eq(List.of(2026L)), eq(false), eq(List.of("ACTIVE")), eq(pageable)))
                .thenReturn(new PageImpl<>(List.of(plan), pageable, 1));

        var result = repository.findPage(List.of(2026L), List.of("ACTIVE"), pageable);

        assertEquals(1, result.getTotalElements());
        verify(jpaRepository).findPageByStatus(List.of(2026L), false, List.of("ACTIVE"), pageable);
