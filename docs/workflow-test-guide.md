...
| 第三步审批人 | `jiaowu_leader` | 教务处领导 | 教务处 | 分管校领导审批 seat，按发起部门 org 解析 |
...
后端当前按 workflow 的 `requester_org_id` 解析这个节点，所以职能部门流程第 3 步不是统一 `admin`，而是发起部门自己的 `_leader` / `_audit2`。
...
4. 系统动态创建分管校领导审批步骤
5. 使用 `keji_leader` 登录，在"我的待办"中通过第三步审批
6. 系统动态创建战略终审步骤
