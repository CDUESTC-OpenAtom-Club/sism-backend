...
- `db-export/`
  - 数据库表与字段导出结果
- `generated/`
  - AI 生成的分片报告、索引清单、修复设计与可重建产物（后续自动生成文档默认落这里）
- `nginx/`
  - 部署用 Nginx 配置
...
3. 自动生成文件统一优先写入 `docs/generated/`，不和手写设计文档混放。
4. 新增文档前先判断它属于“主文档”“生成产物”还是“归档材料”。
5. OpenAPI 规范以项目根目录的 `openapi.json` 为准，不再在 `docs/` 下维护副本。

## 维护建议

1. 新的审批设计或数据库执行文档，优先放在根目录。
2. 一次性任务文档、阶段报告、已完成修复清单，完成后立即归档。
3. `db-export/` 更新时，只覆盖最新快照。
4. OpenAPI 更新统一走项目根目录产物，并用工具刷新 `openapi.json`。
